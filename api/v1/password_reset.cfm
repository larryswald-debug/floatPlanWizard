...
<cftry>
  ...
  <!-- helper to output and stop -->
  <cffunction name="send" access="public" returntype="void" output="true">
    <cfargument name="obj" type="struct" required="true">
    <cfset arguments.obj.BUILD = buildStamp>
    <cfoutput>#serializeJSON(arguments.obj)#</cfoutput>
    <cfabort>
  </cffunction>
  ...
  <cfif action EQ "request">
    ...
    <cfif NOT len(email)>
      <cfset send({ SUCCESS=false, ERROR="MISSING_EMAIL", MESSAGE="Email is required." })>
    </cfif>
  ...
  <cfelseif action EQ "confirm">
    ...
    <cfif NOT len(tokenStr) OR NOT len(newPassword)>
      <cfset send({ SUCCESS=false, ERROR="MISSING_FIELDS", MESSAGE="token and newPassword are required." })>
    </cfif>

    <cfif NOT isNumeric(tokenStr)>
      <cfset send({ SUCCESS=false, ERROR="INVALID_TOKEN", MESSAGE="Reset link is invalid or expired." })>
    </cfif>

    <cfif len(newPassword) LT 8>
      <cfset send({ SUCCESS=false, ERROR="WEAK_PASSWORD", MESSAGE="Password must be at least 8 characters." })>
    </cfif>
    ...
    <cfif reqEpoch LTE 0 OR (nowEpoch - reqEpoch) GT expiresSeconds>
      <cfset send({ SUCCESS=false, ERROR="EXPIRED_TOKEN", MESSAGE="Reset link has expired. Please request a new one." })>
    </cfif>
    ...
    <cfset send({ SUCCESS=true, MESSAGE="Password updated. You can now sign in." })>

  <cfelse>
    <cfset send({ SUCCESS=false, ERROR="INVALID_ACTION", MESSAGE="Invalid action." })>
  </cfif>
