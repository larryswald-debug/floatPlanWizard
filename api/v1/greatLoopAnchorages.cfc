<cfcomponent output="false" hint="Public Great Loop anchorage library JSON API.">

  <cffunction name="handle" access="remote" returntype="void" output="true">
    <cfargument name="action" type="string" required="false" default="">
    <cfargument name="q" type="string" required="false" default="">
    <cfargument name="locationGroup" type="string" required="false" default="">
    ...
    <cfargument name="anchorageType" type="string" required="false" default="">
    ...
    <cftry>
      <cfscript>
        ...
        var anchorageSvc = createAnchorageService();
        var response = {};

        switch (actionName) {
          case "filteroptions":
            response = anchorageSvc.getFilterOptions(filters);
            break;
          case "anchorages":
            response = anchorageSvc.getAnchoragesApiModel(filters, resolveFpwBasePath());
            break;
          default:
            response = buildErrorResponse("Unsupported anchorage library action.");
        }
      ...
      <cfcatch type="any">
        <cfoutput>#serializeJSON(buildErrorResponse("Unable to update anchorage filters. Please try again."))#</cfoutput>
      </cfcatch>
    </cftry>
  ...
  <cffunction name="readFiltersFromUrl" access="private" returntype="struct" output="false">
    <cfscript>
      return {
        "q" = readUrlValue("q"),
        "locationGroup" = readUrlValue("locationGroup"),
        "waterway" = readUrlValue("waterway"),
        "stateProvince" = readUrlValue("stateProvince"),
        "country" = readUrlValue("country"),
        "anchorageType" = readUrlValue("anchorageType"),
