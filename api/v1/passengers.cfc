<cfcomponent output="false">
    ...
    <cffunction name="handle" access="remote" returntype="void" output="true">
        ...
        <cftry>
            ...
            <cfset passengerLimit = 100>

            <cfif structKeyExists(arguments, "limit") AND isNumeric(arguments.limit)>
                <cfset passengerLimit = val(arguments.limit)>
            <cfelseif structKeyExists(url, "limit") AND isNumeric(url.limit)>
                <cfset passengerLimit = val(url.limit)>
            <cfelseif structKeyExists(body, "limit") AND isNumeric(body.limit)>
                <cfset passengerLimit = val(body.limit)>
            </cfif>

            <cfif passengerLimit LTE 0><cfset passengerLimit = 100></cfif>
            <cfif passengerLimit GT 250><cfset passengerLimit = 250></cfif>

            <!-- Load passengers for this user -->
            <cfquery name="qPassengers" datasource="fpw">
            ...
            <cfloop query="qPassengers">
                <cfset passengerStruct = {
                    PASSENGERID   = qPassengers.passId,
                    PASSENGERNAME = qPassengers.name,
                    PHONE         = qPassengers.phone,
                    AGE           = qPassengers.age,
                    GENDER        = qPassengers.gender,
                    NOTES         = qPassengers.notes,
                    HAS_PFD       = qPassengers.pfd
                }>
                <cfset arrayAppend(passengers, passengerStruct)>
            </cfloop>

            <cfset response = {
                SUCCESS = true,
                AUTH    = true,
                COUNT   = arrayLen(passengers),
