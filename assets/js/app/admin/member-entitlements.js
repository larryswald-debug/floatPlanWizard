(function (window, document) {
  ...
  function renderRows() {
    if (!state.items.length) els.tableBody.innerHTML = "<tr><td colspan=\"8\">No entitlements found.</td></tr>";
      ...
      var membershipLabel = item.membershipTypeLabel || item.membershipLevel;
      return "<tr class=\"" + (item.isStripeManaged ? "stripe-managed" : "") + "\"><td>" + escapeHtml(item.memberName || ("Member " + item.userId)) + "<div class=\"small text-muted\">" + escapeHtml(item.memberEmail || "") + " · ID " + item.userId + "</div></td><td>#" + item.entitlementId + " · " + escapeHtml(membershipLabel) + "<div>" + flags + "</div></td><td>" + statusBadge(item.effectiveStatus || item.status) + "<div class=\"small text-muted\">stored: " + escapeHtml(item.status) + "</div></td><td>" + escapeHtml(item.source) + "<div class=\"small text-muted\">" + escapeHtml(item.grantKind || "") + "</div></td><td>" + promoLabel(item) + "<div class=\"small text-muted\">" + escapeHtml(item.promoName || "") + "</div></td><td>" + escapeHtml(dateLabel(item.startsAtUtc)) + "<div class=\"small text-muted\">to " + escapeHtml(dateLabel(item.expiresAtUtc)) + "</div></td><td class=\"small\">" + stripe + "</td><td>" + rowActions(item) + "</td></tr>";
    }).join("");
  ...
  function renderDetail(data) {
    ...
    var effectiveMembershipLabel = access.stripeSubscriptionInterval === "monthly" ? "Monthly Membership" : (access.stripeSubscriptionInterval === "annual" ? "Annual Membership" : "Premium");
    ...
    els.effectiveAccess.textContent = access.hasPremium ? ("Effective access: " + effectiveMembershipLabel + " · controlling source " + safeValue(access.premiumSource) + " · entitlement #" + safeValue(access.premiumEntitlementId) + (access.premiumExpiresAt ? " · expires " + safeValue(access.premiumExpiresAt) : " · open-ended")) : "Effective access: Basic · no currently effective Premium entitlement.";
    els.memberEntitlements.innerHTML = items.length ? items.map(function (item) { return "<tr class=\"" + (item.isStripeManaged ? "stripe-managed" : "") + "\"><td>#" + item.entitlementId + (item.isStripeManaged ? "<br><span class=\"badge text-bg-warning\">Stripe Managed</span>" : "") + "</td><td>" + statusBadge(item.effectiveStatus || item.status) + "</td><td>" + escapeHtml(item.source) + "<br><span class=\"small text-muted\">" + escapeHtml(item.grantKind || "") + "</span></td><td>" + escapeHtml(item.membershipTypeLabel || item.membershipLevel || "—") + "</td><td>" + escapeHtml(dateLabel(item.startsAtUtc)) + "<br>to " + escapeHtml(dateLabel(item.expiresAtUtc)) + "</td><td>" + escapeHtml(item.adminNotes || "") + (item.revocationReason ? "<div class=\"text-danger small\">" + escapeHtml(item.revocationReason) + "</div>" : "") + "</td><td>" + detailEntitlementActions(item) + "</td></tr>"; }).join("") : "<tr><td colspan=\"7\">No entitlement records.</td></tr>";
