...
const TEMPLATE_CODE = "GULF-WEST";
const TEMPLATE_NAME = "Gulf Coast — Tarpon Springs to South Texas";
const sharedState = createCleanupState();
let sharedSupport = null;
const OVERRIDE_POINTS = [
  { lat: 28.09991, lon: -82.82155 },
  { lat: 28.04327, lon: -82.70398 },
  { lat: 27.98514, lon: -82.60122 }
];
...
async function createSupportVessel(page, prefix) {
  const vesselName = `${prefix} Gulf West Vessel`;
...
async function createSupportOperator(page, prefix) {
  const operatorName = `${prefix} Gulf West Operator`;
...
async function createSupportContact(page, prefix) {
  const contactName = `AAA ${prefix} Gulf West Contact`;
  const contactEmail = buildTraceEmail(prefix, "gulf-west-contact");
...
async function configureTemplatePreview(page, direction, expectedStart, expectedEnd, optionalStopCode) {
  ...
  expect(optionsPayload.DATA.optionalStops.length).toBeGreaterThan(0);
  ...
  await expect(page.locator(`[data-stop-code="${optionalStopCode}"]`)).toBeVisible({ timeout: 30000 });
  await page.locator(`[data-stop-code="${optionalStopCode}"]`).click();
...
async function openGeneratedRouteEditor(page, routeCode) {
  await page.locator(`[data-route-code="${routeCode}"] .js-expedition-view-edit`).click();
...
test("GULF-WEST CCW generates, loads, saves, builds float plans, and applies US/Hawaii route-derived persistence", async ({ page }, testInfo) => {
  const consoleErrors = attachConsoleErrorCollector(page);
  const prefix = buildTracePrefix(testInfo, "route-template-gulf-west", "ccw");
  await loginApprovedUser(page);
  const support = await ensureSupportData(page, prefix);

  await configureTemplatePreview(page, "CCW", "Tarpon Springs", "Brownsville", "ship_island_out_and_back");
  ...
  await saveRouteDerivedFloatPlan(page, support, Number(buildPayload.FLOATPLAN_IDS[0]), "US/Hawaii", 20);
  await deleteRouteCard(page, routeCode);

  await assertNoConsoleErrors(consoleErrors);
});

test("GULF-WEST CW generates, loads, saves, builds float plans, and applies America/Puerto_Rico route-derived persistence", async ({ page }, testInfo) => {
  const consoleErrors = attachConsoleErrorCollector(page);
  const prefix = buildTracePrefix(testInfo, "route-template-gulf-west", "cw");
  await loginApprovedUser(page);
  const support = await ensureSupportData(page, prefix);

  await configureTemplatePreview(page, "CW", "Brownsville", "Tarpon Springs", "grand_isle_side_trip");
  ...
  const buildPayload = await buildFloatPlansFromRoute(page, routeCode);
  await saveRouteDerivedFloatPlan(page, support, Number(buildPayload.FLOATPLAN_IDS[0]), "America/Puerto_Rico", 24);
  await deleteRouteCard(page, routeCode);
