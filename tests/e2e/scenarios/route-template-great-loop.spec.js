...
const TEMPLATE_NAME = "Great Loop (Reusable Template v2)";
const sharedState = createCleanupState();
let sharedSupport = null;
const OVERRIDE_POINTS = [
  { lat: 41.8917, lon: -87.6078 },
  { lat: 41.9332, lon: -87.5415 },
  { lat: 41.9806, lon: -87.4382 }
];
...
async function createSupportVessel(page, prefix) {
  const vesselName = `${prefix} Great Loop Vessel`;
...
async function createSupportOperator(page, prefix) {
  const operatorName = `${prefix} Great Loop Operator`;
...
async function createSupportContact(page, prefix) {
  const contactName = `AAA ${prefix} Great Loop Contact`;
  const contactEmail = buildTraceEmail(prefix, "great-loop-contact");
...
test("GL_REUSE_V2 CCW generates, loads, saves, builds float plans, and applies US/Pacific route-derived persistence", async ({ page }, testInfo) => {
  const consoleErrors = attachConsoleErrorCollector(page);
  const prefix = buildTracePrefix(testInfo, "route-template-great-loop", "ccw");
  await loginApprovedUser(page);
  const support = await ensureSupportData(page, prefix);

  await configureTemplatePreview(page, "CCW", "Chicago", "Chicago", "GLD_KEYS_OUTSIDE");
  ...
  await saveRouteDerivedFloatPlan(page, support, Number(buildPayload.FLOATPLAN_IDS[0]), "US/Pacific", 20);
  await deleteRouteCard(page, routeCode);

  await assertNoConsoleErrors(consoleErrors);
});

test("GL_REUSE_V2 CW generates, loads, saves, builds float plans, and applies US/Alaska route-derived persistence", async ({ page }, testInfo) => {
  const consoleErrors = attachConsoleErrorCollector(page);
  const prefix = buildTracePrefix(testInfo, "route-template-great-loop", "cw");
  await loginApprovedUser(page);
  const support = await ensureSupportData(page, prefix);

  await configureTemplatePreview(page, "CW", "Chicago", "Chicago", "GLD_CHAMPLAIN_STLAW");
  ...
  const buildPayload = await buildFloatPlansFromRoute(page, routeCode);
  await saveRouteDerivedFloatPlan(page, support, Number(buildPayload.FLOATPLAN_IDS[0]), "US/Alaska", 24);
  await deleteRouteCard(page, routeCode);
