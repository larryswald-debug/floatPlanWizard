...
<cfset expectedConfirmation = "RUN_DISPOSABLE_SAFE_ARRIVAL_TESTS">
<cfset fixturePattern = "codex-safe-arrival-%">
...
<cfset isLocalDevRequest = listFindNoCase("localhost,127.0.0.1,::1", serverName) GT 0
...
<cfset cleanupResult = { SUCCESS = false, ERROR = "CLEANUP_NOT_RUN" }>

<cftry>
  <cfset runner = createObject("component", "testbox.system.TestBox").init(
    bundles = "fpw.tests.specs.SafeArrivalNotificationServiceSpec"
  )>
    ...
    <cftry>
      <cfquery datasource="fpw">
        ...
        WHERE floatPlanId IN (
          ...
          WHERE floatPlanName LIKE <cfqueryparam value="#fixturePattern#" cfsqltype="cf_sql_varchar">
        )
      </cfquery>
      <cfquery datasource="fpw">
      ...
      <cfquery datasource="fpw">
        ...
        WHERE generated_route_code LIKE 'SAFE_%'
