...
<cfif trim(toString(url.confirm)) NEQ expectedConfirmation>
  ...
  <cfoutput>#serializeJSON({
    ...
    message = "The local Welcome Onboarding test confirmation was not supplied."
  })#</cfoutput>
  <cfabort>
</cfif>
...
<cfif
  qOnboardingColumn.recordCount NEQ 1
  OR val(qOnboardingColumn.compatible_column_count[1]) NEQ 1
>
  <cfheader statuscode="409">
  <cfcontent type="application/json; charset=utf-8" reset="true">
  <cfoutput>#serializeJSON({
    SUCCESS = false,
    ERROR = "WELCOME_ONBOARDING_MIGRATION_REQUIRED",
    message = "Apply and verify the local Welcome Onboarding migration before running this disposable test."
  })#</cfoutput>
  <cfabort>
</cfif>

<cfquery name="qGettingStartedVisibilityColumn" datasource="fpw">
  SELECT COUNT(*) AS compatible_column_count
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = 'FPW'
    AND TABLE_NAME = 'users'
...
<cfif
  qGettingStartedVisibilityColumn.recordCount NEQ 1
  OR val(qGettingStartedVisibilityColumn.compatible_column_count[1]) NEQ 1
>
  <cfheader statuscode="409">
  <cfcontent type="application/json; charset=utf-8" reset="true">
  <cfoutput>#serializeJSON({
    SUCCESS = false,
    ERROR = "GETTING_STARTED_VISIBILITY_MIGRATION_REQUIRED",
    message = "Apply and verify the local Getting Started visibility migration before running this disposable test."
  })#</cfoutput>
  <cfabort>
</cfif>
...
<cfset runnerResponse = {
  ...
  message = "The Welcome Onboarding contract test did not complete."
}>
<cfset cleanupResult = {
  SUCCESS = false,
