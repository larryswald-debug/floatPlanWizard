...
<cfset expectedConfirmation = "RUN_DISPOSABLE_SCHEDULED_ACTUAL_ROUTE_DRAFT_TESTS">
<cfset fixturePattern = "codex-scheduled-actual-route-draft-%">
...
<cftry>
  <cfset runner = createObject("component", "testbox.system.TestBox").init(
    bundles = "fpw.tests.specs.ScheduledActualDepartureRouteDraftSpec"
  )>
  ...
  <cffinally>
    <cftry>
      ...
      <cfquery datasource="fpw">
        ...
        WHERE name LIKE <cfqueryparam value="#fixturePattern#" cfsqltype="cf_sql_varchar">
      </cfquery>
      <cfquery datasource="fpw">
        DELETE FROM vessels
        WHERE CAST(userId AS UNSIGNED) IN (
