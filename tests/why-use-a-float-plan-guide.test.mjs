...
test("hero is text-led with no image contract or reserved image column", () => {
  ...
  const trackRule = cssRule(stylesheet, ".fpw-float-guide-article", "--fpw-float-guide-toc-track-max");
  assert.match(trackRule.declarations, /--fpw-float-guide-toc-track-max:\s*230px/);
  assert.match(trackRule.declarations, /--fpw-float-guide-content-track-max:\s*850px/);
  assert.match(trackRule.declarations, /--fpw-float-guide-reading-gap:\s*clamp\(24px,\s*4vw,\s*52px\)/);
  assert.match(trackRule.declarations, /width:\s*min\(100%,\s*calc\(/);
  assert.match(trackRule.declarations, /margin-inline:\s*auto/);
  const sharedWidthRule = cssRule(stylesheet, ".fpw-float-guide-hero", "width: 100%");
  assert.ok(sharedWidthRule.selectors.includes(".fpw-float-guide-quick-answer"));
  assert.ok(sharedWidthRule.selectors.includes(".fpw-float-guide-reading-layout"));
  const quickAnswerRule = cssRule(stylesheet, ".fpw-float-guide-quick-answer", "margin: 26px auto 30px");
  assert.match(quickAnswerRule.declarations, /margin:\s*26px auto 30px/);
  const readingLayoutRule = cssRule(stylesheet, ".fpw-float-guide-reading-layout", "display: grid");
  assert.match(readingLayoutRule.declarations, /minmax\(0,\s*var\(--fpw-float-guide-content-track-max\)\)/);
  assert.match(readingLayoutRule.declarations, /gap:\s*var\(--fpw-float-guide-reading-gap\)/);
  const responsiveLayoutCss = stylesheet.slice(stylesheet.indexOf("@media (max-width: 1100px)"), stylesheet.indexOf("@media (max-width: 860px)"));
  assert.match(responsiveLayoutCss, /\.fpw-float-guide-article\s*\{[^}]*width:\s*min\(880px,\s*100%\)/);
...
test("print contract hides promotion and protects headings and related cards", () => {
  ...
  const printLayoutRule = cssRule(printCss, ".fpw-float-guide-shell", "display: block");
  assert.ok(printLayoutRule.selectors.includes(".fpw-float-guide-article"));
  assert.ok(printLayoutRule.selectors.includes(".fpw-float-guide-reading-layout"));
  assert.match(printLayoutRule.declarations, /width:\s*100%/);
...
test("page CSS, print behavior, reduced motion, analytics, routes, and sitemap remain contract-backed", () => {
  assert.match(page, /assets\/css\/float-plan-guide\.css\?v=20260830-pillar-full-span-v1/);
