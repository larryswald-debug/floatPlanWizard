...
test("clean route serves complete server-rendered pillar metadata and schema", async ({ page, request }) => {
  ...
  expect(JSON.stringify(graph)).not.toContain("float-plan-hero-bg.png");
  expect(JSON.stringify(graph)).not.toContain("float-plan-guide-hero.webp");

  const socialResponse = await request.get(`${baseUrl}/assets/images/social/floatplanwizard-social-preview-20260730.png`);
...
test("TOC targets, seven visuals, and responsive layouts remain readable from 320 through 1440 CSS pixels", async ({ page }, testInfo) => {
  const pageErrors = collectPageErrors(page);
  const heroImageRequests = [];
  page.on("request", (request) => {
    if (/\/assets\/images\/float-plan\/float-plan-(?:hero-bg\.png|guide-hero\.webp)(?:[?#]|$)/.test(request.url())) {
  ...
  for (const viewport of viewports) {
    ...
    const hero = page.locator(".fpw-float-guide-hero");
    await expect(hero.locator(":scope > .fpw-float-guide-hero__copy"), viewport.name).toHaveCount(1);
    await expect(hero.locator("picture, img, figure, .fpw-float-guide-hero__media"), viewport.name).toHaveCount(0);
    await expect(hero.locator(".fpw-float-guide-eyebrow"), viewport.name).toBeVisible();
    await expect(hero.locator("h1"), viewport.name).toBeVisible();
    await expect(hero.locator(".fpw-float-guide-hero__copy > p:not(.fpw-float-guide-eyebrow)"), viewport.name).toHaveCount(2);
    await expect(hero.locator(".fpw-float-guide-actions"), viewport.name).toBeVisible();
    const heroLayout = await hero.evaluate((element) => {
      const copy = element.querySelector(":scope > .fpw-float-guide-hero__copy");
      const intro = copy.querySelector("p:not(.fpw-float-guide-eyebrow)");
      const heroRect = element.getBoundingClientRect();
      const copyRect = copy.getBoundingClientRect();
      const introRect = intro.getBoundingClientRect();
      const quickAnswerRect = document.querySelector(".fpw-float-guide-quick-answer").getBoundingClientRect();
      const tocRect = document.querySelector(".fpw-float-guide-toc").getBoundingClientRect();
      const contentRect = document.querySelector(".fpw-float-guide-content").getBoundingClientRect();
      const backgroundValues = [
        getComputedStyle(element).backgroundImage,
        getComputedStyle(element, "::before").backgroundImage,
        getComputedStyle(element, "::after").backgroundImage,
        getComputedStyle(copy).backgroundImage
      ];
      return {
        ...
        quickAnswerWidth: quickAnswerRect.width,
        quickAnswerLeft: quickAnswerRect.left,
        quickAnswerRight: quickAnswerRect.right,
        lowerColumnsWidth: contentRect.right - tocRect.left,
        lowerColumnsLeft: tocRect.left,
        lowerColumnsRight: contentRect.right,
        heightDifference: Math.abs(heroRect.height - copyRect.height),
        introWidth: introRect.width,
        usesRasterBackground: backgroundValues.some((value) => value.includes("url("))
      };
    });
    expect(heroLayout.display, viewport.name).toBe("block");
    expect(heroLayout.copyWidth, viewport.name).toBeGreaterThanOrEqual(heroLayout.heroWidth - 3);
    expect(heroLayout.heightDifference, viewport.name).toBeLessThanOrEqual(3);
    expect(heroLayout.introWidth, viewport.name).toBeLessThanOrEqual(850);
    expect(heroLayout.usesRasterBackground, viewport.name).toBe(false);
    expect(Math.abs(heroLayout.heroWidth - heroLayout.quickAnswerWidth), viewport.name).toBeLessThanOrEqual(2);
    expect(Math.abs(heroLayout.heroWidth - heroLayout.lowerColumnsWidth), viewport.name).toBeLessThanOrEqual(2);
    expect(Math.abs(heroLayout.heroLeft - heroLayout.lowerColumnsLeft), viewport.name).toBeLessThanOrEqual(2);
    expect(Math.abs(heroLayout.heroRight - heroLayout.lowerColumnsRight), viewport.name).toBeLessThanOrEqual(2);
    expect(Math.abs(heroLayout.quickAnswerLeft - heroLayout.lowerColumnsLeft), viewport.name).toBeLessThanOrEqual(2);
    expect(Math.abs(heroLayout.quickAnswerRight - heroLayout.lowerColumnsRight), viewport.name).toBeLessThanOrEqual(2);
    if (viewport.width > 1100) expect(heroLayout.heroWidth, viewport.name).toBeLessThanOrEqual(1134);
    if (viewport.width <= 1100 && viewport.width > 880) expect(heroLayout.heroWidth, viewport.name).toBeLessThanOrEqual(882);
...
test("print mode preserves the article, visuals, checklist, sources, and review date while hiding site controls", async ({ page }, testInfo) => {
  ...
  await expect(page.locator(".fpw-float-guide-hero__media")).toHaveCount(0);
