...
test("hero is text-led with no image contract or reserved image column", () => {
  ...
  assert.match(heroRule.declarations, /max-width:\s*850px/);
  assert.match(heroRule.declarations, /margin-inline:\s*auto/);
  assert.doesNotMatch(heroRule.declarations, /grid-template-columns/);
  const quickAnswerRule = cssRule(stylesheet, ".fpw-float-guide-quick-answer", "max-width: 850px");
  ...
  assert.match(readingLayoutRule.declarations, /minmax\(0,\s*850px\)/);
  ...
  assert.match(responsiveLayoutCss, /\.fpw-float-guide-hero,\s*\.fpw-float-guide-quick-answer\s*\{[^}]*max-width:\s*880px/);
...
test("page CSS, print behavior, reduced motion, analytics, routes, and sitemap remain contract-backed", () => {
  assert.match(page, /assets\/css\/float-plan-guide\.css\?v=20260830-pillar-no-hero-width-v1/);
