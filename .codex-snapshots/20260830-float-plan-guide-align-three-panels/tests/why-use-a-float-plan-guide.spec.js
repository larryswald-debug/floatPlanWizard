...
test("TOC targets, seven visuals, and responsive layouts remain readable from 320 through 1440 CSS pixels", async ({ page }, testInfo) => {
  ...
  for (const viewport of viewports) {
    ...
    expect(Math.abs(heroLayout.heroWidth - heroLayout.contentWidth), viewport.name).toBeLessThanOrEqual(2);
    if (viewport.width > 1100) expect(heroLayout.heroWidth, viewport.name).toBeLessThanOrEqual(852);
